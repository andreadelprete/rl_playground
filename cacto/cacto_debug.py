...
'''
Simple script debugging the CACTO algorithm on a 1d single integrator.
'''
...
if __name__=="__main__":
    ...
    U = actor(X[ind,:])
    X_next = jax.vmap(dynamic_aug)(X[ind,:], U)
    V_X = critic(X[ind,:])
    V_X_next = critic(X_next[ind,:])
    # for i in ind:
    #     plt.plot([X[i,0], X_next[i,0]], [U[i], V_X_next[i]])
