...
'''
Simple script testing CACTO with and w/o Sobolev learning, on a 1d single integrator.
'''
...
def plot_actor(t=0, show=True, plot_u_ocp=True, plot_running_cost=True):
    ...
    if(plot_running_cost): plt.plot(X_grid, running_cost, label="running cost", alpha=0.5)
...
if __name__=="__main__":
    ...
    critic_sob = NeuralNetwork("Critic Sob.", n_x+1, 1, critic_layers, learning_rate_critic_sob, RANDOM_SEED, 
                        sobolev_weight)
    actor      = ActorNetwork("Policy", n_x+1, n_u, actor_layers, cost_aug, critic, dynamic_aug, 
                        learning_rate_actor, RANDOM_SEED)
    actor_sob  = ActorNetwork("Policy Sob.", n_x+1, n_u, actor_layers, cost_aug, critic_sob, dynamic_aug, 
                        learning_rate_actor, RANDOM_SEED)
    ...
    V_sob = np.empty((N_grid,CACTO_ITERS+1))
    ...
    for k in range(CACTO_ITERS):
        ...
        print("Start training critic with Sobolev")
        for i in range(max_critic_iter):
            critic_loss = critic_sob.train_sobolev(X_aug, V_buffer, dVdx_buffer, 
                                        critic_updates, minibatch_size=minibatch_size)
            print("Iter", i, "\tCritic loss", critic_loss)
            if(i%10==11):
                plot_critic(critic_sob, t=1, show=True)
        print("Critic training has finished")
        for t in range(1, N, N):
            print("Critic for t=", t)
            plot_critic(critic_sob, t, show=False)
            plt.legend()
            plut.saveFigure("Iter_"+str(k)+"_critic_sob_t_"+str(t))
            if(SHOW_PLOTS): plt.show()
        ...
        print("Start training actor SOBOLEV minimizing Q function")
        for i in range(max_actor_iter):
            actor_loss = actor_sob.train(X_aug, actor_updates, minibatch_size=minibatch_size) 
            # U = actor_sob(X_aug)
            # dQdu = jax.vmap(grad_Q)(U, X_aug)
            # dQdu_norm = jnp.mean(jnp.abs(dQdu))
            print("Iter", i, "\tActor loss", actor_loss) #, "\tdQdu", dQdu_norm)
        print("Actor SOBOLEV training finished")
        ...
        plot_actor(t=0, show=False, plot_u_ocp=False, plot_running_cost=False)
        plt.xlabel("State x")
        plt.plot(X_grid, actor_pre_train, ':', label='Actor pre-training', alpha=0.5)
        plt.plot(X_grid, actor_sob(X_grid_aug), label='Actor Sobolev', alpha=0.5)
        plt.legend()
        ...
        for (i, x_init) in enumerate(X_grid):
            X_guess, U_guess, V_guess[i] = compute_initial_guess(x_init, 0, N, n_x, n_u, actor_sob)
            sol = ocp.solve(x_init, N, X_guess, U_guess)
            V_sob[i,k+1] = sol.value(ocp.cost)
        print("Finished solving OCP's")
        print("Average optimal cost:\n", jnp.mean(V[:,k+1]))
        print("Average optimal cost with Sob:\n", jnp.mean(V_sob[:,k+1]))

        plt.figure()
        plt.plot(X_grid, V[:,0], ':', label="Value V(x,t=0), iter 0", alpha=0.5)
        for i in range(1, k+2):
            plt.plot(X_grid, V[:,i], label="Value V(x,t=0), iter "+str(i), alpha=0.5)
            plt.plot(X_grid, V_sob[:,i], label="Value V(x,t=0) Sob., iter "+str(i), alpha=0.5)
